...
- Prisma schema: see below for the exact schema to use. Use `prisma migrate dev` and create a seed script.
...
- Node **22+** is required.
- Initialize `package.json` for `cinebook-server` with Typescript, Express, Prisma, Zod, Redis etc.

## Exact Prisma Schema (from CineBook-Build-Plan.md §3)
```prisma
